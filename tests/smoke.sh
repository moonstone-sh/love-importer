...
"$LUA_BIN" src/main.lua inspect "$TMP/root" | grep -q "kind:       normalized_root"

"$LUA_BIN" src/main.lua import "$TMP/root" --version 11.5 --target linux-x86_64-gnu --out "$TMP/out-root"
...
"$LUA_BIN" src/main.lua import "$TMP/app/love.app" --version 11.5 --target darwin-aarch64 --out "$TMP/out-app" --skip-run-check
...
"$LUA_BIN" src/main.lua import "$TMP/love-11.5-macos.zip" --version 11.5 --target darwin-aarch64 --out "$TMP/out-zip" --skip-run-check --clear-quarantine
...
"$LUA_BIN" src/main.lua import-system "$TMP/root/bin/love" --version 11.5 --target linux-x86_64-gnu --out "$TMP/out-system" --local-only --skip-run-check
...
if "$LUA_BIN" src/main.lua import "$TMP/love.AppImage" --version 11.5 --target linux-x86_64-gnu --out "$TMP/out-bad" >/tmp/love-importer-bad.out 2>&1; then
